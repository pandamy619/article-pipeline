...
Пайплайн собирает статьи из нескольких источников, отбирает и переписывает их локальной нейросетью на русском, а перед публикацией в Telegram-группу каждый пост проходит **проверку человеком**.
...
- **Публикация** (`app/src/publisher`) — после одобрения постит в группу через Bot API, сохраняет `message_id` и статус.
